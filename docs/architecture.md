...
- **Key interfaces:**
  - `async merge_audio(input_paths: list[str], gap_ms: int, crossfade_ms: int, output_format: str, input_durations: list[float] | None = None) -> tuple[bytes, str]` — Discovers the ffmpeg binary via `imageio_ffmpeg.get_ffmpeg_exe()`, builds the ffmpeg command (concat demuxer for plain concat; `aevalsrc=0` silence for gap; a deterministic `afade`+`adelay`+`amix` crossfade for crossfade, which needs `input_durations` — `acrossfade` was dropped per ISSUE-033 for non-deterministic truncation on short clips), invokes via `asyncio.create_subprocess_exec`, renders to a seekable temp file (not `pipe:1`, so WAV RIFF size headers are finalized — ISSUE-032) and reads the bytes back, raises `SubprocessError` on nonzero exit with stderr excerpt.
